<script lang="ts">
	...
	// Email filter stores
	const emailFilterText = writable<string>('');
	const parsedEmailFilter = derived(emailFilterText, $emailFilterText =>
		parseEmailFilter($emailFilterText)
	);
	...
	$: {
		...
		filterTimeout = window.setTimeout(() => {
			const filtered = filterCustomers($originalData, $searchFilters, $parsedUsernameFilter, $parsedEmailFilter);
...
<main class="min-h-screen bg-gray-50 p-6 transition-all duration-300">
	<div class="mx-auto max-w-7xl">
		...
		{:else}
			...
			<div class="mb-6 rounded-lg bg-white p-6 shadow-md">
				<div class="mb-4">
					<h3 class="text-lg font-medium text-gray-900">Filter by Emails</h3>
					<p class="mt-1 text-sm text-gray-600">
						Enter email addresses separated by commas or new lines. Only customers with matching email addresses will be shown.
					</p>
				</div>
				<div class="space-y-4">
					<div>
						<label for="email-filter" class="block text-sm font-medium text-gray-700">
							Email Addresses
						</label>
						<textarea
							id="email-filter"
