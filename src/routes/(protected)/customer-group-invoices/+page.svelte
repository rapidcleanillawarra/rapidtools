...
<script lang="ts">
		...
		try {
			...
			const mappedInvoices = orders_data.Order.map((order: any) => {
				...
				const completeStatus =
					order.CompleteStatus != null && String(order.CompleteStatus).trim() !== ''
						? String(order.CompleteStatus).trim()
						: '—';
	...
	// Function to open invoice printout page
	function openInvoicePrintout() {
		try {
			if (!$invoices || $invoices.length === 0) {
				toastError('No invoices to print');
				return;
			}
			...
			toastSuccess(`Opening printout for ${$invoices.length} invoices`);
		} catch (error) {
			console.error('Failed to open invoice printout:', error);
			toastError('Failed to open invoice printout');
		}
	}
