...
<div class="container mx-auto px-4 py-8" in:fade>
  <div class="bg-white rounded-lg shadow-lg overflow-hidden">
    <div class="px-6 py-4 border-b border-gray-200">
      <h1 class="text-xl font-semibold">Claimed Rebates</h1>
      <p class="text-gray-600">View and manage claimed rebates.</p>
    </div>
      ...
      <div class="flex flex-wrap items-center gap-4">
        ...
        {#if startDate || endDate}
          <div class="text-sm text-gray-500">
            {#if startDate && endDate}
              Showing claimed rebates from {startDate} to {endDate}
            {:else if startDate}
              Showing claimed rebates from {startDate} onwards
            {:else if endDate}
              Showing claimed rebates up to {endDate}
            {/if}
          </div>
        {/if}
      </div>
      ...
      {#if apiData}
        <div class="space-y-4">
          <div class="flex justify-between items-center">
            <h3 class="text-lg font-medium text-gray-900">Claimed Rebates Data</h3>
          ...
          {:else}
            <div class="text-center py-8">
              <p class="text-gray-500">No claimed rebates found for the selected date range.</p>
            </div>
          {/if}
        </div>
      {:else if !isLoading && !error}
        <div class="text-center py-12">
          <p class="text-gray-500">Select a date range and click "Filter" to view claimed rebates data.</p>
        </div>
      {/if}
    </div>
  </div>
