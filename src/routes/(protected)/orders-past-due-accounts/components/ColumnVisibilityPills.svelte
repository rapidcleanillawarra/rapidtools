...
<div class="mt-6">
	<h3 class="mb-3 text-sm font-medium text-gray-700 dark:text-gray-300">Visible Columns:</h3>
	<div class="flex flex-wrap gap-2">
		{#each columns.filter((column) => column.key !== 'customer') as column}
			<button
				type="button"
				on:click={() => dispatch('toggle', { key: column.key })}
				class="inline-flex items-center rounded-full px-3 py-1.5 text-sm font-medium transition-colors duration-200 {columnVisibility[
					column.key
				]
	...
	<p class="mt-2 text-xs text-gray-500 dark:text-gray-400">Customer is always shown.</p>
</div>
