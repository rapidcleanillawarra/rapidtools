<script lang="ts">
	...
	function handlePrintBarcode() {
		...
		window.open(`${base}/product-barcode/print?skus=${encodeURIComponent(skus)}`, '_blank');
	}
	...
	function applyPurchasePricePercentChange() {
		...
		for (const sku of selected) {
			const base =
				originalMap.get(sku)?.purchase_price ??
	...
	function applyMarkupAddition() {
		...
		for (const sku of selected) {
			const base =
				originalMap.get(sku)?.markup ?? $products.find((p: any) => p?.sku === sku)?.markup;
			const baseNum = toNumber(base, 0);
	...
	function getMainImage(product: any): string | null {
		...
		const main = images.find((img: any) => (img?.Name || img?.name || '').toLowerCase() === 'main');
	...
	function openPhotoViewer(product: any) {
		...
		photoViewerIndex = mainIndex >= 0 && uniqueUrls[mainIndex] ? mainIndex : 0;
