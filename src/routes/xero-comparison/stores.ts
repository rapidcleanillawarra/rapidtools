...
export const invoiceIdsError = writable<string | null>(null);
...
// Filter stores
export const invoiceIds = writable<string>('');
export const parsedInvoiceIds = writable<string[]>([]);
...
// Helper function to parse invoice IDs
function parseInvoiceIds(input: string): string[] {
  return input
    .split(/[,\n\r\s]+/) // Split by comma, newline, or space
    .map(id => id.trim())
...
export function validateFilters(): boolean {
  ...
  invoiceIdsError.set(null);

  const currentInvoiceIds = get(invoiceIds).trim();
  
  // If invoice IDs are provided, they take priority - make other filters optional
  if (currentInvoiceIds) {
    // Parse and validate invoice IDs
    const ids = parseInvoiceIds(currentInvoiceIds);
    if (ids.length === 0) {
      invoiceIdsError.set('Please enter valid invoice IDs');
      isValid = false;
    } else {
      parsedInvoiceIds.set(ids);
      // When invoice IDs are provided, other filters become optional
    ...
    // Original validation logic when no invoice IDs
