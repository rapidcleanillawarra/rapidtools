<script lang="ts">
  ...
  type JobTypeFilter = 'pickup' | 'return' | 'all';
  ...
  const jobTypeChips: { key: JobTypeFilter; label: string }[] = [
  ...
  let jobTypeFilter = $state<JobTypeFilter>('all');
  ...
  let filteredRows = $derived(
    rows.filter((row) => {
      ...
      if (jobTypeFilter !== 'all' && row.job_status !== jobTypeFilter) return false;
  ...
  <header class="sticky top-0 z-20 bg-white border-b border-gray-200 shadow-sm">
    <div class="px-4 pt-4 pb-3">
      ...
      <div class="mt-2 flex flex-wrap gap-2">
        {#each jobTypeChips as chip (chip.key)}
          <button
            type="button"
            class="rounded-full px-3 py-1.5 text-sm font-medium transition
              {jobTypeFilter === chip.key
              ? 'bg-blue-600 text-white'
              : 'bg-blue-50 text-blue-800 hover:bg-blue-100'}"
            onclick={() => (jobTypeFilter = chip.key)}
          >
            {chip.label}
          </button>
        {/each}
  ...
  <main class="flex-1 px-4 py-4 pb-8">
    ...
    {:else if filteredRows.length === 0}
      <div class="py-16 text-center text-gray-500 text-sm">
        {rows.length === 0 ? 'No pickup or return jobs right now' : 'No matches for these filters'}
      </div>
    {:else}
      <ul class="space-y-3">
        {#each filteredRows as row (row.workshop.id)}
          <li>
            <div
